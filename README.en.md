...
inside the target profile directory via `dsh plugin`; the target profile is initialized
automatically on first use, and once installed, the plugin (which declares `dsh.bundle`)
is added to the profile's bundles layer automatically — restart DSH (`dsh web`) to pick
it up.
...
> Git-hosted plugins build on install via their `prepare` script. If pnpm blocks
> that build, add the exact key pnpm printed to `allowBuilds` in
> `<profile>/pnpm-workspace.yaml`, then re-run the command above.
...
The package is removed from the profile's dependencies and bundles layer; restart DSH
to take effect.
